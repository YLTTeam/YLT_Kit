...
- (void)reloadData {
    ...
    for (AppSectionModel *model in self.pageModel.page) {
...
- (AppPageModel *)pageModel {
    if (!_pageModel) {
        ...
        _pageModel = [AppPageModel mj_objectWithKeyValues:allPages];
        NSArray<NSDictionary *> *pages = allPages[@"page"];
        [_pageModel.page enumerateObjectsUsingBlock:^(AppSectionModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
